...
if(!isAllowed("pSolutionEdit")) {die("У Вас недостаточно прав для просмотра этой страницы");}
if (!isset($curr_page)) $curr_page=0;
$_SESSION["pageback"]="solutions.php?curr_page=$curr_page";

if(!($sortby)) $sortby='solutions_pos'; else {$sortby=explode(" ",$sortby);$sortby=$sortby[0];}
...
$mainquery="SELECT
			...
			{$PREFFIX}_solutions.solutions_pos,
			{$PREFFIX}_page.page_code,
			{$PREFFIX}_page.page_name,
			{$PREFFIX}_page.page_active
			FROM
			{$PREFFIX}_solutions
			INNER JOIN {$PREFFIX}_page ON {$PREFFIX}_solutions.page_code = {$PREFFIX}_page.page_code
...
$query="insert into {$PREFFIX}_page (page_name,  page_active, page_type) values('$page_name',0,'solutions')";
...
if(!$err)
  {
      mysql_query("commit;");
    //для постраничного-------------------------------------------------------------
     $result=mysql_query($mainquery) or die("Incorrect solutions Query ".mainquery) ;
  }
...
if ($oper=="D")
{
  while (list($key,$value)=each($_POST))
  {
      ...
      if ( ($varName=="C")&&($value=="on"))
      {
         ...
         $query = "delete from {$PREFFIX}_solutions where page_code=$varCode";
         $resultdel = mysql_query($query) or $err+=10;
      }
  }
}//oper=D
...
if (!strcasecmp($oper,"U"))
{
  ...
  while (list($key,$value)=each($_POST))
  {
   ...
   RenumeratePos("{$PREFFIX}_solutions","solutions_code","solutions_pos","page_code",$page_code);
...
{
if (confirm('Вы уверены, что хотите удалить проектное решение?'))
 {
    document.data.oper.value=a;
    document.data.submit();
 }
...
<table Border=0 CellSpacing=0 CellPadding=0 width=100%>
  <tr><td class=pageline>
     <div class=wmiddletext><a href="main.php">Администрирование сайта</a> &#187; <a>Проектное решение</a></div>
  </td></tr>
</table>
&nbsp;
...
<table class=grayhead Border=0 CellSpacing=0 CellPadding=0 >
 <tr class=normaltext>
  <td ><div ><h4>Проектные решения</h4></div></td>
  <td align=right class=wmiddletext><a class=submenu onclick="displayform(this,'добавить решение')">добавить решение </a></td>
 </tr>
</table>
...
<table Border=0 CellSpacing=0 class=pagebluetable CellPadding=0>
 ...
 <tr><td colspan=10 class=blueheadcolor><center><div class=normaltext>ДОБАВИТЬ РЕШЕНИЕ</div></center></td></tr>
...
<tr height=30 >
 ...
 <!--  <tr height=30 >
    ...
    <td width=5></td>
    <td><input name='solutions_url' type=text style="width:250px" class=smalltext></td> 
 </tr> -->
 </table>
 </td>
       ...
       <tr>
        <td class=helptd>
             ...
             <div class=ssmalltext>
                Заполните поля формы и нажмите кнопку "добавить решение". </a>
             </div>
 ...
 <tr><td colspan=3 height=1 bgcolor=#ffffff></td></tr>
 <tr> <td colspan=10 id=blueheadcolor><center><input type=button onClick=Send('I') value='добавить решение'  class=smalltext> </td></tr>
</table>
</div>


<table Border=0 CellSpacing=0 class=pagebluetable CellPadding=0 >
 <tr><td class=lmenutext height=30 bgcolor=#ffffff align="center">Проектные решения</td></tr>
</table>

<?php
 if(intval($err)<=-10) echo"<div class=smalltext align=center style='color:red;'>Ошибка при изменении решения</div>";
 if(intval($err)>=10) echo"<div class=smalltext align=center style='color:red;'>Ошибка при удалении решения</div>";
 if(intval($err)==1) echo"<div class=smalltext align=center style='color:red;'>Ошибка при добавлении решения</div>";
?>
...
<table Border=0 CellSpacing=1 class=bluetable CellPadding=4 width=100%>
  <tr class=lmenutext height=20 align=center bgcolor=#ffffff>
    <td width=20>&nbsp;</td>
    <td width="80"><?=SortTitle("Позиция","solutions_pos",$sortby,$sortdir);?></td>
...
$num_rows=mysql_num_rows($res);  //Количество строк
  if($num_rows)
  {
      ...
      for ($x=$start_pos; $x<$end_pos; $x++)
      {
         ...
         echo"<TD align=left class=smalltext ondblclick='change_line(\"$checkname\",\"F#$checkname#solutions_pos#int\");' id=\"F#$checkname#solutions_pos#int\">".Show($solutions_pos)."</TD>\n";
