...
//  echo"$query";
  $result=mysql_query($query) or die("Cannot Insert Picture ".$query) ;
//для постраничного-------------------------------------------------------------
...
echo"<input type=hidden name=langindex value=\"$langindex\">";
?>
...
<?
$picquery="SELECT picture_code,page_code,picsmall,picbig,picpos,piccomment FROM {$PREFFIX}_picture where (page_code=$page_code) and (language_code=$langindex) order by $sortby $realsortdir";
$res=mysql_query ($picquery) or die ("Cannot Select Picture $picquery");
...
<?php
  if($num_rows)
  {
    ...
    {
    list($picture_code,$p_c,$picsmall,$picbig,$picpos,$piccomment)=mysql_fetch_array($res);
